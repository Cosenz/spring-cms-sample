window.onload = () => {
  ...
  const getUI = (baseUrl, resources, configUI, configSecurity) => {
    const ui = SwaggerUIBundle({
      ...
      dom_node: null,
      spec: {},
      // Usare urls e commentare url se si vuole produrre la documentazione dal codice e non usare lo yml
      //url: "yml/sample-api.yml",
      urls: resources,
      /*--------------------------------------------*\
       * Plugin system
      \*--------------------------------------------*/
  ...
  const getBaseURL = () => {
    const urlMatches = /(.*)\/index.html.*/.exec(window.location.href);
    return urlMatches[1];
  };
